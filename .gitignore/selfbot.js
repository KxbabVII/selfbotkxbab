...
bot.on('ready', () => {
  ...
  bot.user.setPresence({
      game: {
          ...
          type: "STREAMING",
          url: "https://www.twitch.tv/insomniaselfbot"
      }
  });
});
...
if (command === `${prefix}help`) {
  ...
  .setDescription("Menu principal : Page 1")  
  .setColor("RANDOM")
  .addBlankField()  
...
if (command === `${prefix}mod`) {
  ...
  let helpEmbed = new Discord.RichEmbed()
  .setTitle("**𝙄𝙉𝙎𝙊𝙈𝙉𝙄𝘼 𝙎𝙀𝙇𝙁𝘽𝙊𝙏**")
  .setDescription("Menu principal : Page 2")  
  .setColor("RANDOM")
  .addBlankField()  
  .addField("Prefix : /", "Pour afficher la page d'aide : /help")
  .addField("**Modération :**", "`ban [@USER] : Bannit l'utilisateur mentionné\nkick [@USER] : Kick l'utilisateur mentionné\nsondage [TEXT] : Crée un sondage en Embed\nannonce [TEXT]: Crée une annonce en Embed\nclear [CHIFFRE] : Supprime un nombre de message spécifié\ndeletec : Supprime tout les salons d'un serveur (Très dangereux)\ndeleter : Supprime tout les rôles d'un serveur (Très dangereux)\nname [TEXT] : Change le nom du serveur\nicon : Change la PDP du serveur`")
  .addBlankField()
  .setThumbnail(message.author.avatarURL)
  .setImage("https://cdn.discordapp.com/attachments/595155050704142337/604043339448320030/MOSHED-2019-7-1-14-1-53.gif")
  .setFooter(`𝙄𝙉𝙎𝙊𝙈𝙉𝙄𝘼 𝙎𝙀𝙇𝙁𝘽𝙊𝙏 | Commande mod`, message.author.avatarURL);
  message.channel.send(helpEmbed)

  console.log("Commande mod exécuté")
...
if(!message.member.hasPermission("KICK_MEMBERS")) {
  let kickPermissionEmbed = new Discord.RichEmbed()
...
.setFooter(`𝙄𝙉𝙎𝙊𝙈𝙉𝙄𝘼 𝙎𝙀𝙇𝙁𝘽𝙊𝙏 | Commande ping`, message.author.avatarURL)
...
if (command === `${prefix}name`) {
  ...
  let askErrorEmbed = new Discord.RichEmbed()
  .setDescription("Veuillez choisir un nom pour le serveur | Syntaxe : ?name [TEXT]")
  .setColor("RANDOM")
  if (!args[0]) return message.reply(askErrorEmbed)

  if(!message.member.hasPermission("MANAGE_SERVER")) {
    let namePermissionEmbed = new Discord.RichEmbed()
    .setDescription("Vous n'avez pas la permission d'utiliser cette commande ! (Permission MANAGE_SERVER requise)")
    .setColor("RANDOM")
    return message.channel.send(namePermissionEmbed);
  ...
  let successNameChange = new Discord.RichEmbed()
  .setDescription(`Le nom du serveur est désormais ${serverName}`)
  .setColor("RANDOM")
  .setFooter(`𝙄𝙉𝙎𝙊𝙈𝙉𝙄𝘼 𝙎𝙀𝙇𝙁𝘽𝙊𝙏 | Commande name`, message.author.avatarURL)
  message.channel.send(successNameChange)

  console.log("Commande name exécuté")
...
if (command === `${prefix}icon`) {
  ...
  let askErrorEmbed = new Discord.RichEmbed()
  .setDescription("Veuillez choisir une PDP pour le serveur | Syntaxe : ?icon[LIEN]")
  .setColor("RANDOM")
  if (!args[0]) return message.reply(askErrorEmbed)
  ...
  let serverIcon = args.join(" ");
  message.guild.setIcon(serverIcon)
  
  let successIconChange = new Discord.RichEmbed()
  .setDescription(`La PDP du serveur a bien été changé !`)
  .setColor("RANDOM")
  .setFooter(`𝙄𝙉𝙎𝙊𝙈𝙉𝙄𝘼 𝙎𝙀𝙇𝙁𝘽𝙊𝙏 | Commande icon`, message.author.avatarURL)
  message.channel.send(successIconChange)

  console.log("Commande icon exécuté")

}
//Deleter
if(command === `${prefix}deleter`) {
  if (message.author.id != id) return;
  if (message.author.bot) return;
  message.delete()

  if(!message.member.hasPermission("ADMINISTRATOR")) {
    let deleterPermissionEmbed = new Discord.RichEmbed()
    .setDescription("Vous n'avez pas la permission d'utiliser cette commande ! (Permission ADMINISTRATOR requise)")
    .setColor("RANDOM")
    message.channel.send(deleterPermissionEmbed);

  }

    let deleterSuccessEmbed = new Discord.RichEmbed()
    .setDescription("Suppression de tout les rôles en cours.")
    .setColor("RANDOM")

    message.channel.send(deleterSuccessEmbed)

    message.guild.roles.map(r => r.delete())

    console.log("Commande deleter exécuté")

}

//Deletec
if(command === `${prefix}deletec`) {
  if (message.author.id != id) return;
  if (message.author.bot) return;
  message.delete()

  if(!message.member.hasPermission("ADMINISTRATOR")) {
    let deletecPermissionEmbed = new Discord.RichEmbed()
    .setDescription("Vous n'avez pas la permission d'utiliser cette commande ! (Permission ADMINISTRATOR requise)")
    .setColor("RANDOM")
    message.channel.send(deletecPermissionEmbed);

  }

    let deletecSuccessEmbed = new Discord.RichEmbed()
    .setDescription("Suppression de tout les salons en cours.")
    .setColor("RANDOM")

    message.channel.send(deletecSuccessEmbed)

    message.guild.channels.map(c => c.delete())

    console.log("Commande deletec exécuté")
